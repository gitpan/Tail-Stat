...
test_tcp(
	client => sub {
		...
		alarm 3;
		print $s "stats x\n";
		is $s->getline => "cache_status_expired: 6\r\n";
		is $s->getline => "cache_status_hit: 75\r\n";
		is $s->getline => "cache_status_miss: 12\r\n";
		is $s->getline => "cache_status_stale: 4\r\n";
		is $s->getline => "cache_status_updating: 3\r\n";
		...
		is $s->getline => "last_request_time: 40.641\r\n";
		is $s->getline => "last_upstream_request: 73\r\n";
		is $s->getline => "last_upstream_time: 13.775\r\n";
		is $s->getline => "malformed_request: 0\r\n";
		is $s->getline => "upstream_next: 1\r\n";
		is $s->getline => "upstream_request: 73\r\n";
		alarm 0;
	},
	server => sub {
		my $port = shift;
		$ENV{ PERL5LIB } = join ':', @INC;
		exec qq($^X $bin -b$db -f -l$port --log-level=error -p$pid -w1 nginx x:$log);
	},
);
